<template>
  <div class="person-container__wrapper">
    <div class="person-container">
      <div class="person-image__wrapper">
        <img class="person-picture" :src="getRandomImage()" alt />
        <h2 class="person-name">Name: {{ person.name }}</h2>
      </div>
      <div class="person-information__wrapper">
        <div class="person-information">
          <div><span>Gender </span> {{ person.gender }}</div>
          <div><span> DOB </span>{{ person.birth_year }}</div>
          <div><span>Mass </span>{{ person.mass }}</div>
          <div><span>Height </span>{{ person.height }}</div>
        </div>
        <div class="person-information">
          <div><span>Hair color </span>{{ person.hair_color }}</div>
          <div><span>Skin color </span>{{ person.skin_color }}</div>
          <div><span>eye color </span>{{ person.eye_color }}</div>
        </div>
      </div>
    </div>
